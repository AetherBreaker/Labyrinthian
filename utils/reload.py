    ...
    *sequences: MutableSequence[:class:`str`]:
    ...
    while True:
        ...
        for (
            candidate,
            *_,
        ) in nonempty_seqs:  # find merge candidates among sequence heads
            logger.debug(
                "[MRO MERGE] Checking candidate %s against %s", candidate, nonempty_seqs
            )
            ...
            )  # Circular imports and other shenanigans
...
def reload_child_modules(module: ModuleType) -> bool:
    ...
    for module_name in import_hierarchy[:0:-1]:
        ...
        try:
            ...
            sys.modules.update(
                (name, data.module) for name, data in import_mapping.items()
            )
            return False
