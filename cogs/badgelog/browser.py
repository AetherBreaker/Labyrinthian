...
class CharSelect(UIBase):
    ...
    async def _init(self, inter: disnake.ApplicationCommandInteraction):
        self.embed = disnake.Embed.from_dict(
            {
                "title": "Badge browser",
                "description": f"Select a character."
                if self.user is None
                else f"Select one of <@{self.user.id}>'s characters below.",
                ...
                "fields": [
                    ...
                    {"name": "Class Levels:", "value": "\u200B", "inline": True},
                    {"name": f"Total Levels: ", "value": "\u200B", "inline": True},
                ],
            }
        )
        self.charlist = (
            await self.bot.sdb[f"BLCharList_{self.guild.id}"]
            .find({"user": str(self.owner.id if self.user is None else self.user.id)})
            .sort("character", DESCENDING)
            .to_list(None)
        )
        self._refresh_character_select()
        if self.charlist:
            await inter.response.send_message(
                embed=self.embed, view=self, ephemeral=self.ephem
            )
        else:
    ...
    def _refresh_character_select(self):
        ...
        for char in reversed(self.charlist):  # display highest-first
            self.select_char.add_option(
                label=char["character"], value=char["character"]
            )
    ...
    async def select_char(
        ...
        if charname is None:
            ...
            return
        Log = LogBrowser(
            self.bot, self.owner, self.guild, charname, self.charlist, self.user
        )
...
class LogBrowser(UIBase):
    ...
    async def refresh_content(self, inter: disnake.Interaction):
        if inter.response.is_done():
            await inter.edit_original_message(
                embed=self.embeds[self.embed_count], view=self
            )
        else:
    ...
    async def construct_embeds(self, charname: str) -> List[disnake.Embed]:
        char = await self.bot.dbcache.find_one(
            f"BLCharList_{self.guild.id}",
            {"user": str(self.user.id), "character": charname},
        )
        badgelog = (
            await self.bot.sdb[f"BadgeLogMaster_{self.guild.id}"]
            .find({"charRefId": ObjectId(char["_id"]), "user": str(self.user.id)})
        ...
        if badgelog:
            ...
            while badgelog:
                ...
                while badgelog:
                    ...
                    if (
                        len(result) >= 6000
                        or len(Embed["fields"][3]["value"]) >= 400
                        or not len(badgelog) > 0
                    ):
                        if len(logstr) > 1:
    ...
    @disnake.ui.select(
        placeholder="Select Character", row=4, min_values=1, max_values=1
    )
    async def select_char(
        self, select: disnake.ui.Select, inter: disnake.MessageInteraction
    ):
    ...
    def _refresh_character_select(self):
        ...
        for char in reversed(self.charlist):  # display highest-first
            selected = (
                self.firstchar is not None and self.firstchar in char["character"]
            )
            self.select_char.add_option(
                label=char["character"], value=char["character"], default=selected
            )
