...
class Configs(commands.Cog):
    ...
    async def adddmrole(
        ...
        await self.bot.sdb["srvconf"].replace_one(
            {"guild": str(inter.guild.id)}, srvconf, True
        )
        await inter.send("The DM roles have been updated.", ephemeral=True)

    @dmroles.sub_command(name="remove")
    @commands.cooldown(3, 30.0, type=commands.BucketType.user)
    async def removedmrole(
        self,
        inter: disnake.ApplicationCommandInteraction,
        role: disnake.Role,
        role2: disnake.Role = None,
    ...
    async def addclass(
        self,
        inter: disnake.ApplicationCommandInteraction,
        classname: str = commands.Param(name="class"),
        classname2: str = commands.Param(name="class2", default=None),
        classname3: str = commands.Param(name="class3", default=None),
        classname4: str = commands.Param(name="class4", default=None),
    ):
    ...
    @admin.sub_command()
    @commands.cooldown(3, 30.0, type=commands.BucketType.user)
    async def badgetemplate(
        self, inter: disnake.ApplicationCommandInteraction, templatedict: str
    ):
        try:
            bdgtemplate = loads(templatedict)
        except JSONDecodeError:
            return await inter.response.send_message(
                f"Error: Template not a valid JSON\n Traceback:```py\n{traceback.print_exc()}```"
            )
        for itr, x in enumerate(bdgtemplate.keys()):
            if itr > 20:
                return await inter.response.send_message(
                    "Error: Template has too many entries"
                )
            elif x != str(itr + 1):
                return await inter.response.send_message(
                    "Error: Template keys are not a range of 1 through 20"
                )
        if all([isinstance(x, (int, float)) for x in bdgtemplate.values()]):
            ...
            return await inter.response.send_message(
                "Error: Template value is not of type integer or float"
            )
