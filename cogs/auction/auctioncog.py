...
class AuctionHouse(commands.Cog):
    ...
    async def listing_handler(self, inter: disnake.MessageInteraction):
        ...
        charlist: List[str] = await self.bot.sdb[
            f"BLCharList_{inter.guild.id}"
        ].distinct("character", {"user": str(inter.author.id)})
        listingdat: Dict[str, Any] = await self.bot.dbcache.find_one(
            "auction_listings", {"listingid": str(inter.message.id)}
        )
        if len(charlist) < 1:
            await inter.send(
                f"<@{inter.author.id}> You don't have any characters!\nYou must have a registered character to use the auction house!",
                ephemeral=True,
            )
            return
        elif listingdat is None:
            await inter.send(
                f"<@{inter.author.id}> This listing seems to be broken, or its data has been lost, please contact the listing owner or a staff member for assistance.",
                ephemeral=True,
            )
            return
        ...
        #     await inter.send(f"<@{inter.author.id}> You can't bid on your own auctions!", ephemeral=True)
            ...
            listingdat["enddate"].replace(tzinfo=timezone.utc) < disnake.utils.utcnow()
        ):
            await inter.message.delete()
            await inter.send(
                f"<@{inter.author.id}> Sorry, that auction has ended!", ephemeral=True
            )
        ...
        if not listingfinished:
            ...
            try:
                modal_inter: disnake.ModalInteraction = await self.bot.wait_for(
                    "modal_submit",
                    check=lambda i: i.custom_id == f"{rand}auction_char_modal"
            ...
            LHandler = ListingHandler(
                ...
                config=srvconf,
                charname=modal_inter.data["components"][0]["components"][0]["values"][
                    0
                ],
            )
            if (
                inter.component.custom_id == buttonids[0]
            ):  # Handle bid lowest interactions
                listingfinished = await LHandler.bid_lowest()
            elif (
                inter.component.custom_id == buttonids[1]
            ):  # Handle custom bid interactions
                listingfinished = await LHandler.bid_custom()
                if LHandler.error:
                    return
            elif (
                inter.component.custom_id == buttonids[2]
            ):  # Handle buy now interactions
                listingfinished = await LHandler.buy_now()
            if not listingfinished:
                AHandler = AuctionHandler(
                    bot=self.bot, auctiondata=LHandler.auctiondata, config=srvconf
                )
                await AHandler.update_listing_tracker(embed=LHandler.embed)
                await AHandler.log_bid()
                await modal_inter.response.edit_message(
                    embed=LHandler.embed, components=ListingActionRow(listingdat)
                )
                self.bot.sdb["auction_listings"].replace_one(
                    {"listingid": str(inter.message.id)}, LHandler.auctiondata
                )
                return
        ...
        if listingfinished:
            ...
            await inter.message.delete()
            await self.bot.sdb["auction_listings"].delete_one(
                {"listingid": str(inter.message.id)}
            )
        else:
            return
